...
visualizeGammaFits<-function(mean.vals,gamma.parameters,nGenes=21000, censorPoint = NULL,
  ...
  sim.mean.vals<-sampleMeanValuesQuantiles(gamma.parameters, nGenes, censorPoint)
...
sampleMeanValuesQuantiles <- function(gamma.parameters, nGenes = 21000, censorPoint = NULL) {
  ...
  # Third Gamma component
  nGamma3 <- nGenes - nZeros - nGamma1 - nGamma2
  quantiles.c3 <- seq(1 / (nGamma3 + 1), 1 - 1 / (nGamma3 + 1), by = 1 / (nGamma3 + 1))
  vals <- c(vals, qgamma(quantiles.c3, shape = gamma.parameters$s3, rate = gamma.parameters$r3))

  # Exclude values below the censor point
  print(censorPoint)
  if (!is.null(censorPoint)) {
    vals <- vals[vals >= censorPoint]
  }
...
```{r}
datasetID <- "EFO:0009922_UBERON:0003126_CL:0000786"
datasetID <- strsplit(datasetID, split = "_")[[1]]
...
for (factor_name in names(increasedCensorPoints)) {
  ...
  p <- visualizeGammaFits(normMeanValues$mean[normMeanValues$matrix == "complete"],
                            gammaFits[gammaFits$matrix == "complete",],
                            nGenes = numGenesKept,
                            censorPoint = currentCensorPoint)
  ...
  filename <- paste0("C:/Users/Cem/Downloads/3rd_gamma_fits/trachea_plasmacell_with_cutoff_updated/", "gamma_fits_plot_", factor_name, ".png")
