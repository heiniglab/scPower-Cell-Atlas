...
```{r}
  ...
  idToName <- ""
  cellCount <- 0
  chunk_size <- 10000 # Max number of gene to keep in each chunk
  chunk_index <- 0
```
...
```{r}
  for(datasetID in datasetCollectionCombinedID){
      ...
      result <- tryCatch({
        ...
        # After subsetting, split the data into chunks by genes
        gene.ids <- rownames(dataset@assays[[1]]@counts)
        # split these gene IDs into chunks
        chunks <- split(gene.ids, ceiling(seq_along(gene.ids)/chunk_size))
