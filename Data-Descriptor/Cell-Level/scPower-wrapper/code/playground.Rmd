...
<!-- when using another dataset, change: results & dataset path, num.genes.kept -->
...
getNSamples <- function(dataset) {
  ...
  for (column in sampleColumnNames) {
    if (!is.null(dataset@meta.data[[column]])) {
      return(length(levels(dataset@meta.data[[column]])))
    }
  }
...
validationByUMICounts <- function(observedGeneCountsDF, nSamples, gammaLinearFitsDF, dispFunParamDF) {
...
mainProcessing <- function(dataset) {
  ...
  estimates <- validationByUMICounts(observedGeneCountsDF, nSamples, gammaLinearFitsDF, dispFunParamDF)
