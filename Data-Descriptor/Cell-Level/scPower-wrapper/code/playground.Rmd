...
```{r}
  ...
  for (hash in hashAsList){
    ...
    result_gammaLinear <- dbGetQuery(connectionInstance, query_gammaLinear)
    result_dispFun <- dbGetQuery(connectionInstance, query_dispFun)
    result_powerResults <- dbGetQuery(connectionInstance, query_powerResults)
  ...
  # en son descriptiveparams ve hashleri main tablo olarak yazdım
  # result tablolarından gammalinear, dispfun ve poweresults'ı pull ettim, variablelarda tutuyorum
  # daha append etmedim
  # problem: neden örneğin son result_dispFun 2 instance içeriyor
  # bunun dışında bir de geneRanks dahil edilecek
```
