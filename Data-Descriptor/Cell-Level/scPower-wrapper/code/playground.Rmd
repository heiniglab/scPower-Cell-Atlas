...
validationByUMICounts <- function(observedGeneCounts, nSamples, gammaLinearFits, dispFunParam) {
  ...
  estimates$deviation <- abs(estimates$expressed.genes - estimates$estimated.counts) / estimates$expressed.genes
...
```{r}
  ...
  # RMSE values summarized across the different subsampled matrices
  complete_RMSEs <- c()
...
```{r}
  ...
  outputResults(data.frame(mean_RMSE_complete, mean_RMSE_subsampled75, mean_RMSE_subsampled50, mean_RMSE_subsampled25), "RMSE_results")
