...
```{r}
  ...
  for(i in 1:nrow(estimates)){
    exp.probs <- scPower::estimate.exp.prob.count.param(
      ...
      gamma.mixed.fits = head(gammaLinearFitsDF, 54),
      ct = estimates$cell.type[i],
      disp.fun.param = head(dispFunParamDF, 9),
      min.counts = 10,
      perc.indiv = 0.5
    )
...
```{r}
  # Setting the cell type specification
  gammaFitsDF <- head(gammaFitsDF, 8)
  gammaFitsDF$ct <- "New_ct"
  disp.fun.general.new <- head(dispFunParamDF, 2) %>% select(asymptDisp, extraPois)
  ...
  power <- power.sameReadDepth.restrictedDoublets(nSamples = 100, nCells = 1500,
                                                  ...
                                                  cellsPerLane = 20000,
                                                  gamma.parameters = gammaFitsDF[gammaFitsDF$matrix == "complete",],
                                                  ct = "New_ct",
