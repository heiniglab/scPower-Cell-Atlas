...
Then for the validation step:
```{r}
  # Firstly append gammaLinearFitsDF to the already existing one in the scPower library
  #gammaLinearFitsDF <- rbind(scPower::gamma.mixed.fits, gammaLinearFitsDF)

  # stepping into validation
  estimates.allSamples <- NULL
  estimates <- head(observedGeneCountsDF, 8)
  estimates$estimated.counts <- NA
  
  i <- 1
  exp.probs <- estimateExpProbCountParam(
    nSamples = nSamples,
    nCellsCt = estimates$num.cells[i] / nSamples,
    meanCellCounts = estimates$meanUMI[i],
    gamma.mixed.fits = head(gammaLinearFitsDF, 12),
    ct = estimates$cell.type[i],
    disp.fun.param = head(dispFunParamDF, 2),
    min.counts = 10,
    perc.indiv = 0.5
  )
...
```{r}
  estimateExpProbCountParam <- function(nSamples,nCellsCt,meanCellCounts,
                                        gamma.mixed.fits,
                                        ct,disp.fun.param,
    ...
    #Sample means values
    gene.means<-sampleMeanValuesQuantiles(gamma.parameters,nGenes)
...
```{r}
  sampleMeanValuesQuantiles <- function(gamma.parameters, nGenes=21000){
    ...
    vals<-rep(0,nZeros)
    #vals<-lapply(nZeros, function(x) rep(0, x))
    ...
    #to <- 1-1/(nGamma1+1)
    #by <- 1/(nGamma1+1)
    #quantiles.c1<-mapply(seq, from = from, to = to, by = by)
    #scale <- 1/gamma.parameters$r1
    
    # Convert any non-numeric elements in quantiles.c1 to numeric
    #quantiles.c1 <- lapply(quantiles.c1, as.numeric)
    #vals <- c(vals, qgamma(quantiles.c1, shape = gamma.parameters$s1, scale = scale))
