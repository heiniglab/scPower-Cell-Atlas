...
# dispersionFunctionResults: asymptDisp, extraPois
# powerResults: name, powerDetect, exp.probs. power, sampleSize, totalCells, usableCells
              # multipletFraction, ctCells, expressedGenes
mergeFirstIteration <- function(cellCount, numberOfAssays, numberOfTissues, numberOfCellTypes, 
                                assayID, tissueID, cellTypeID, gammaLinearFits, dispersionFunctionResults, powerResults) {
  ...
  resultingDataFrame <- data.frame(datasetBodySpecific, resultTableSpecific, gammaLinearFits, dispersionFunctionResults, powerResults)

  print("Merging of the finals informations done successfully.")
  return(resultingDataFrame)
}

mergeFinalStatus <- function(firstIterationResults, estimates) {
  return(data.frame(firstIterationResults, estimates))
}
...
```{r}
  ...
  firstIterationResultsDF <- data.frame()
```
...
```{r}
  for(datasetID in datasetCollectionCombinedID){
    ...
    # Descriptive parameters passed:
      # cellCount, #assays, #tissues, #cellTypes,
      # assayID, tissueID, cellTypeID
      # gammaLinearFits, dispFunEstimation
    # Power calculation: simplified gamma model without optimizing the read depth 
    firstIterationResult <- mergeFirstIteration(cellCount,
                                                ...
                                                dispFunEstimation,
                                                power)
    firstIterationResultsDF <- rbind(firstIterationResultsDF, firstIterationResult)
  }
```
...
```{r}
  ...
  # result tables
  #   gammaLinearFits
  #   dispersionFunctionResults
  #   powerResults
  #   estimates: estimating gene counts
  #   rsq: R squared error rate of the estimate
  resultingDataFrame <- mergeFinalStatus(firstIterationResultsDF, estimates)
  
  # Writing final data frame, different results merged into, to database table named "priorsResult"
